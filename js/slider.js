	...
	slider: function(obj) {
		...
		Slider.prototype.buildHtml = function() {
			$.getJSON("inc/" + obj.src, function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					html += "<img src=\"" + data[i]["src"] + "\" alt=\"" + data[i]["title"] + "\"/>"
					html += "</a>";
					html += "</li>";
				}
				html += "</ul>";
				...
				liWidth = $li.width();
				liLength = $li.length;
				$sliderNav = $(".slider-nav", that);
				// $sliderNav.addClass("success")
				var count = 0;
				$li.each(function() {
		...
		Slider.prototype.drag = function() {
			...
			$ul.on("click touchstart touchmove touchend mousedown mousemove mouseup", function(e) {
				...
				if (e.type == "click" && Math.abs(dragged) > 1) {
					e.preventDefault();
